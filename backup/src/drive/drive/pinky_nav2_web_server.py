...
def get_static_dir():
    try:
        return os.path.join(get_package_share_directory("drive"), "pinky", "scripts")
    except PackageNotFoundError:
        return os.path.dirname(os.path.abspath(__file__))


BASE_DIR = get_static_dir()
...
class Nav2WebBridge(Node):
    def __init__(self):
        ...
        self.declare_parameter("save_map_path", "/home/pinky/current_map")
    ...
    def slam_save_map(self, name: str) -> tuple[bool, str]:
        """
        /slam_toolbox/save_map 호출: 현재 SLAM 맵을 pgm+yaml로 저장.
        name: 파일 이름 (확장자 없이). 기본값은 nav launch가 읽는 current_map.
        """
        ...
        req = SaveMap.Request()
        save_map_path = self.get_parameter("save_map_path").value
        map_path = name if os.path.isabs(name) else save_map_path
        req.name = String(data=map_path)

        future = self.save_map_client.call_async(req)
        self.get_logger().info(f"[WEB] Requested SLAM save_map: name='{map_path}'")
...
def api_slam_save_map():
    ...
    ok, map_path = ros_node.slam_save_map(name)
    return jsonify({"success": ok, "name": map_path})
