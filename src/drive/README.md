...
속도는 `0.20rad/s`, 헤딩 허용오차는 2도입니다. 경로나 TF가 없으면 방향을
추측하지 않고 5cm 직선 후진하며 제자리 회전 동작은 사용하지 않습니다.
후진 직후 로컬·글로벌 코스트맵을 모두 초기화하고 최신 관측으로 다시 생성할 수
있도록 0.4초 동안 기다린 뒤 원래 목표 경로를 재계획합니다. 경로 실행이 다시
실패하면 후진부터 같은 순서를 목표당 최대 4회 반복합니다. 정적 맵·keepout 경계나
불가능한 목표 자세는 코스트맵 초기화로 없어지지 않으므로, 4회 실패하면 Nav2 목표를
실패로 종료하여 중앙관제가 새 목표를 판단할 수 있게 합니다. 목표 도착, 명령 취소,
자율관제 중지 또는 비상정지가 발생하면 그보다 먼저 종료합니다.

복구 후진은 오래된 코스트맵 셀 때문에 생략되지 않습니다. 최신 후방 LiDAR,
비상정지와 차량 간 충돌방지 게이트는 그대로 우선합니다. 컨트롤러의 연속 실패
허용 시간은 0.15초이며, 충돌 예측이 반복되면 중간 재추종 없이 바로 이 복구
순서로 넘어갑니다.
