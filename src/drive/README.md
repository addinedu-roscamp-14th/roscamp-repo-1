...
`costmap_scan_filter`는 AMCL과 복구 노드가 사용하는 원본 `scan`은 유지하고,
코스트맵 전용 `scan_costmap`을 발행합니다. 인접 빔 군집 또는 직전 프레임에서
반복 확인되지 않은 단일 고립 반환은 `inf`로 바꿔 clearing ray로 사용합니다.
따라서 차량이 없는 빈 위치에 한 프레임만 남는 점은 제거하면서 벽, 기둥,
상대 차량 충돌 레이어와 위치추정 입력은 유지합니다.

상대 차량은 위치 기반 `other_robot_layer`에서만 표현하도록, 최신 상대 AMCL
위치를 LiDAR 프레임으로 변환하고 그 중심 반경 0.22m 안의 실제 LiDAR 반환은
`scan_costmap`에서 제외합니다. 차량이 이동하면 해당 빔이 clearing ray가 되어
이전 위치의 흔적을 지우고, 별도 상대 차량 레이어가 새 위치만 표시합니다. 상대
차량 위치가 1초 이상 오래됐거나 TF가 없으면 안전상 LiDAR 반환을 제거하지
않습니다. 상대 차량 레이어 자체도 이전 원판과 외곽 링을 모두 clearing합니다.
TF 시간 불일치로 clearing PointCloud를 만들지 못하는 경우에도 이전 차량의 map
좌표를 이용해 로컬·글로벌 `ClearCostmapAroundPose` 서비스를 호출합니다. 따라서
차량이 지나간 자리에 규칙적인 2cm 격자 원판이 고정되어 남지 않습니다.

`adaptive_lidar_recovery`는 `agv1`·`agv2` 각각의 차량 내부에서 실행되며
Nav2 플래너 또는 컨트롤러 실패 시 Behavior Tree가 호출합니다. LiDAR 후방
여유가 10cm 이상이면 최대 10cm, 10cm 미만이면 최대 5cm 후진을 시도합니다.
후방 여유가 10cm 미만이면 5cm를 선택합니다. 오래된 코스트맵이 뒤쪽을 막고
있어도 후진을 생략하지 않지만, 최신 후방 LiDAR가 없거나 실제 물체가 1cm까지
접근한 경우에는 물리 충돌을 피하기 위해 정지합니다. 각 복구 시도마다 최신 후방
LiDAR 여유로 5cm 또는 10cm를 선택합니다. 최신 global path에서 차량에 가장
가까운 지점과 그 앞 12cm 구간의 접선 방향을 구하고, 후진이 끝날 때 차량 헤딩이
그 경로 진행 방향을 바라보도록 오도메트리 피드백 조향을 적용합니다. 최대 조향
속도는 `0.30rad/s`, 헤딩 허용오차는 2도입니다. 경로나 TF가 없으면 방향을
추측하지 않고 5cm 직선 후진하며 제자리 회전 동작은 사용하지 않습니다.
후진 직후 로컬·글로벌 코스트맵을 모두 초기화하고 두 코스트맵의 새 전체 grid를
확인한 뒤 0.2초 안정화 시간을 두고 원래 목표 경로를 재계획합니다. 경로 실행이 다시
실패하면 후진부터 같은 순서를 횟수 제한 없이 반복합니다. 각 후진 뒤에는 반드시
로컬·글로벌 코스트맵의 새 grid를 확인하고 경로를 다시 생성합니다. 목표 도착,
명령 취소, 자율관제 중지 또는 비상정지가 발생하면 반복을 종료합니다.

복구 후진은 오래된 코스트맵 셀 때문에 생략되지 않습니다. 최신 후방 LiDAR,
비상정지와 차량 간 충돌방지 게이트는 그대로 우선합니다. 컨트롤러는 충돌 중
정지 상태를 유지하면서 1 Hz 전역 재계획을 최소 한 번 더 받을 수 있도록 연속
실패를 1.5초까지 기다립니다. 새 경로도 계속 충돌하면 위 복구 순서로 넘어갑니다.

중앙관제 주행, 주차, RViz 직접 목표가 모두 같은 차량 로컬 복구를 사용합니다.
노드는 상대 토픽 `scan`, `odom`, `cmd_vel_nav`과 차량 namespace 내부의
`adaptive_lidar_recovery` 서비스를 사용하며 정상 차량 launch에 포함됩니다.
복구 속도는 Nav2 velocity smoother를 지난 뒤 `cmd_vel_safe_input` 안전 게이트로
전달됩니다.
