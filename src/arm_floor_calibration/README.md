...
기존 Pick/Place 코드를 수정하지 않고, 다음 값만 실기체에서 교시하는 ROS 2
패키지입니다.
...
colcon build --symlink-install --packages-select arm arm_floor_calibration
...
```bash
ros2 launch arm_floor_calibration floor_calibration.launch.py \
...
기본 결과 파일은 실행한 디렉터리 기준
`calibration_results/floor_calibration.yaml`입니다. 매 캡처 직후 원본 샘플이
저장되므로 H 계산 전에 노드가 종료돼도 이미 저장된 결과는 남습니다. 노드를 다시
