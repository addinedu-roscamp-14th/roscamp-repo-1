...
class CameraToMapBridge(Node):
    ...
    def on_pixel_point(self, msg):
        ...
        pose_msg = self.build_pose_msg(msg, map_x, map_y)
        json_msg = self.build_json_msg(msg, camera_pixel, map_x, map_y)

        self.pose_pub.publish(pose_msg)
        self.json_pub.publish(json_msg)

        self.get_logger().info(
            f'Converted camera_pixel={self.round_list(camera_pixel, 3)} '
            f'to map_xy={[round(map_x, 6), round(map_y, 6)]}'
        )
    ...
    def build_pose_msg(self, source_msg, map_x, map_y):
    ...
    def build_json_msg(self, source_msg, camera_pixel, map_x, map_y):
        payload = {
            ...
            'camera_pixel': {
                'u': round(camera_pixel[0], 3),
                'v': round(camera_pixel[1], 3),
            },
            'map_pose': {
                'x': round(map_x, 6),
                'y': round(map_y, 6),
