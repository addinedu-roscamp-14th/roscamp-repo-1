...
전에 현재 위치에서 왼쪽으로 `90°` 회전합니다. 회전 Nav2 목표가 성공해야 다음
단계로 넘어가며, 회전이 끝나면 회전된 차량 전방으로 `0.10m` 직진한 뒤 최종
목적지 경로를 시작합니다. 회전 목표는 현재 위치를 그대로 유지하고 ROS의 양의
yaw(반시계 방향)로 생성합니다. 기본 각도는 `b1_exit_left_turn_deg`, 직진 거리는
`b1_exit_forward_distance_m` 파라미터로 관리합니다. 회전 또는 직진이 실패하면
최종 목적지 경로를 시작하지 않습니다.
