"""Launch the package-local non-moving floor calibration workflow."""
...
def resolve_runtime(context):
    """Resolve local paths and create the calibrated camera transform."""
    ...
    handeye_node = Node(
        ...
        name='floor_calibration_handeye_transform',
        arguments=[
            '--x', str(translation['x']),
            '--y', str(translation['y']),
            '--z', str(translation['z']),
...
def generate_launch_description():
    """Create camera, local TF, ArUco, and calibration nodes."""
    package_share = Path(
    ...
    arguments = [
        ...
        DeclareLaunchArgument(
            'params_file',
            default_value=str(
                package_share / 'config' / 'floor_calibrator.yaml'
            ),
        ),
        DeclareLaunchArgument(
            'output_file',
    ...
    robot_tf = Node(
        ...
        name='floor_calibration_robot_state_publisher',
        namespace='arm',
        output='screen',
        parameters=[{
    ...
    detector = Node(
        ...
        executable='calibration_aruco_pose_publisher',
        namespace='arm',
