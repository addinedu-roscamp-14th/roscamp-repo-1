...
탑다운 카메라
  ...
  -> Eye-in-Hand TF로 arm2/base_link 기준 pose 변환
  -> XYZ와 회전 정밀 보정
  -> 기존 하강/그리퍼 닫기/상승 동작
```
...
└── launch/
    ├── arm2_robot_tf.launch.py              # URDF와 실제 관절 TF
    ├── arm2_gripper_aruco.launch.py         # /dev/video4와 ArUco 검출
    ├── arm2_handeye_calibration.launch.py   # Eye-in-Hand 샘플 수집
    ├── arm2_handeye_publish.launch.py       # 저장한 Hand-Eye TF 발행
    └── arm2_container_pick.launch.py        # 추적/TF/집기 코디네이터 통합 실행
```
...
```text
config/arm2/arm2_arm_camera_info.yaml
```
...
```bash
ros2 launch arm2 arm2_gripper_aruco.launch.py \
...
```bash
ros2 topic echo /arm2/gripper_camera/aruco_pose
...
```text
입력: /arm2/gripper_camera/image_raw (sensor_msgs/Image)
입력: /arm2/gripper_camera/camera_info (sensor_msgs/CameraInfo)
출력: /arm2/gripper_camera/aruco_pose (geometry_msgs/PoseStamped)
출력: /arm2/gripper_camera/aruco_annotated (sensor_msgs/Image)
TF: arm2/gripper_camera_optical_frame -> arm2/handeye_target
```
...
```bash
ros2 launch arm2 arm2_handeye_calibration.launch.py \
    camera_info_url:=config/arm2/arm2_arm_camera_info.yaml \
    marker_id:=0 \
    marker_size_m:=0.026
```
...
```bash
ros2 launch arm2 arm2_handeye_publish.launch.py \
  name:=arm2_jetcobot_eye_in_hand
```

저장된 결과가 발행되면 다음 TF 연결이 완성됩니다.

```text
arm2/base_link -> arm2/TCP
arm2/TCP -> arm2/gripper_camera_optical_frame
arm2/gripper_camera_optical_frame -> arm2/container_marker
```

따라서 다음 명령으로 컨테이너 마커의 `arm2/base_link` 기준 6D pose를 확인할 수 있습니다.
...
ros2 run tf2_ros tf2_echo arm2/base_link arm2/container_marker
```

## 5. 컨테이너 추적과 Pick dry-run

컨테이너의 `DICT_5X5_50`, ID `0`, 검은 사각형 가로·세로 `26 mm` 마커를 추적합니다.
처음에는 로봇을 움직이지 않고 계산된 목표만 확인합니다. 다른 카메라 및 로봇 시리얼
노드는 모두 종료한 뒤 실행합니다.

```bash
ros2 launch arm2 arm2_container_pick.launch.py \
  camera_info_url:=config/arm2/arm2_arm_camera_info.yaml \
  marker_id:=0 \
  marker_size_m:=0.026 \
...
ros2 topic echo /arm2/container_pick/marker_pose
ros2 topic echo /arm2/container_pick/grasp_pose
ros2 topic echo /arm2/container_pick/pregrasp_pose
...
통과해야 합니다.
26 mm 단일 마커의 회전값은 흔들릴 수 있어 dry-run은 회전 spread를 `30도`까지,
실제 실행은 `15도`까지 허용합니다.

인터페이스:

```text
서비스: /arm2/pick_container (std_srvs/Trigger)
서비스: /arm2/preview_pregrasp (std_srvs/Trigger)
서비스: /arm2/move_to_pregrasp (std_srvs/Trigger)
서비스: /arm2/stop_pick (std_srvs/Trigger)
출력: /arm2/container_pick/status (std_msgs/String)
출력: /arm2/container_pick/marker_pose (geometry_msgs/PoseStamped)
출력: /arm2/container_pick/grasp_pose (geometry_msgs/PoseStamped)
출력: /arm2/container_pick/pregrasp_pose (geometry_msgs/PoseStamped)
```
...
```text
config/arm2/arm2_container_pick.yaml
```

현재 26 mm 단일 마커는 위치는 안정적이지만 평면 회전값이 흔들릴 수 있으므로, 정렬된
...
```bash
ros2 service call /arm2/preview_pregrasp std_srvs/srv/Trigger '{}'
```
...
cd ~/poter_ws
colcon build --symlink-install --packages-select \
  jetcobot_description jetcobot_moveit_config arm2
source install/setup.bash
```
...
`/arm2/pick_container`는 요청 직후 기존 마커 샘플을 버리고 새 샘플 5개가 안정화될
때까지 최대 20초 기다립니다. 호출 순간 마지막 샘플이 3초보다 오래됐더라도 새 프레임이
들어오면 목표를 잠근 뒤 그리퍼 열기, pregrasp 이동과 정렬, 하강, 그리퍼 닫기와
상승을 실행합니다. 첫 실기기 테스트에서는 항상 `/arm2/move_to_pregrasp`의 위치와
자세를 먼저 확인합니다.
...
Topic: /joint_states
Service: /arm2/gripper/open
Service: /arm2/gripper/close
Service: /arm2/stop_robot
```
...
```bash
ros2 run arm2 click_pick_place --ros-args \
