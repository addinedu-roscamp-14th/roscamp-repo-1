"""Launch declared-stack relocation with MoveIt and eye-in-hand ArUco."""
...
def generate_launch_description():
    """Create the physical declared-stack relocation graph."""
    moveit_share = Path(
        get_package_share_directory('jetcobot_moveit_config')
    )
    handeye_share = Path(get_package_share_directory('easy_handeye2'))
    package_share = Path(get_package_share_directory('arm_relocation2'))
    ...
    arguments = [
        ...
        DeclareLaunchArgument(
            'source_stack', default_value='[1, 2, 3, 4]'
        ),
        DeclareLaunchArgument('empty_stack', default_value='[5]'),
        DeclareLaunchArgument('pick_marker', default_value='2'),
        DeclareLaunchArgument('place_marker', default_value='6'),
        ...
        DeclareLaunchArgument(
            ...
            default_value=str(
                package_share
                / 'config'
                / 'container_pick_place_relocation2.yaml'
            ),
        ),
    ...
    detector = Node(
        package='arm_relocation2',
        executable='stack_aruco_pose_publisher',
        name='stack_aruco_pose_publisher',
        namespace='arm',
        output='screen',
        parameters=[{
            'camera_frame_id': LaunchConfiguration('camera_frame_id'),
    ...
    coordinator = Node(
        package='arm_relocation2',
        executable='stack_pick_place_coordinator',
        # Retain the selector used by config/arm/container_pick_place.yaml.
        name='container_pick_place_coordinator',
        namespace='arm',
        output='screen',
        parameters=[
            LaunchConfiguration('params_file'),
            LaunchConfiguration('stack_params_file'),
            {
                'base_frame': 'base_link',
                'execute_motion': True,
                'motion_backend': 'moveit',
                'source_stack': ParameterValue(
                    LaunchConfiguration('source_stack'), value_type=str
                ),
                'empty_stack': ParameterValue(
                    LaunchConfiguration('empty_stack'), value_type=str
                ),
                'target_container_id': ParameterValue(
                    LaunchConfiguration('pick_marker'), value_type=int
                ),
                'final_place_marker_id': ParameterValue(
