...
API의 `/api/v1/navigation/park`로 요청을 보냅니다. 차량은 각자 전용 스팟(agv1
`park_red`, agv2 `parking_yellow`)으로 실제 후진 주차합니다. 명령 없이도 차량이
...
대시보드가 이를 `agv1`과 `agv2` action으로 확장해 병렬 전송합니다. 두 차량이
...
명령을 순차 전송하며 과거 화물 목록은 변경하지 않습니다. 이 단계는 AGV 이동
계획이며 로봇팔 상차·하차 서비스 호출은 별도 작업입니다.

`agv1을 B-1로 보내고 agv2를 A구역으로 보내`처럼 두 차량이 서로의 목적 구역을
